...
| `filter_tf_idf`        | manipulation | Filter low tf_idf words from `TermDocumentMatrix`/`DocumentTermMatrix` |
...
Here we use `filter_documents`' default (a document must have a rox/column sum greater than 1) to eliminate the warning:
