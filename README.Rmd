...
| `filter_words` | Remove words from `TermDocumentMatrix`/`DocumentTermMatrix` |
| `select_documents` | Remove documents from `TermDocumentMatrix`/`DocumentTermMatrix` |
...
On a smaller `r nrow(presidential_debates_2012)` rows these are the time comparisons between **gofastr** adn **tm** using `Sys.time`:
